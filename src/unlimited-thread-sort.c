...
void *merge_and_print(void *voidBlock)
{
    //printf("Created thread\n");
    merge_sort(voidBlock);
}
...
void *merge_sort(void *voidBlock)
{
    ...
    if (block->size > SPLIT)
    {
        ...
        int leftThread = pthread_create(&t1, NULL, merge_and_print, &left_block);
        merge_sort(&right_block);

        if (leftThread != 0)
        {
            // if thread creation fails, then merge the left block
            //__sync_fetch_and_add(&failCount, 1);
            merge_sort(&left_block);
        }
        else
        {
            // thread creation success, so wait for completion
...
int main(int argc, char *argv[])
{
    ...
    //printf("Used %i threads\n", count);
    //printf("Failed at creating %i threads\n", failCount);
    free(block.data);
    exit(EXIT_SUCCESS);
}