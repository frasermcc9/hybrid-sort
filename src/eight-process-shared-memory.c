...
#define MMAP_ARGS PROT_READ | PROT_WRITE, MAP_ANONYMOUS | MAP_SHARED, -1, 0
...
void fork_merge(struct block *block, int depth)
{
    struct block left_block;
    left_block.size = block->size / 2;
    left_block.data = block->data;
    ...
    if (depth < 3)
    {
        ...
        {
            // in child
            fork_merge(&right_block, depth + 1);
            exit(EXIT_SUCCESS);
        }
    }
    else
...
int main(int argc, char *argv[])
{
    ...
    struct block *block = mmap(NULL, sizeof(int) * (int)pow(2, size) + sizeof(int), MMAP_ARGS);
    block->size = (int)pow(2, size);
    block->data = mmap(NULL, block->size * sizeof(int), MMAP_ARGS);
    if (block->data == NULL)
    {
