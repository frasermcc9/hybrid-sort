...
/* Merge sort the data. */
void *merge_sort(void *voidBlock)
{
    struct block *block = (struct block *)voidBlock;
    if (block->size > SPLIT)
    {
...
void *prepare_sort(void *args)
{
    struct prepare_args *prepared_args = (struct prepare_args *)args;

    if (prepared_args->block->size > SPLIT)
    {
        int depth = prepared_args->depth;
        struct block *block = prepared_args->block;
        pthread_t t1;
        ...
        if (depth < use_depth)
        {
            struct prepare_args left_args, right_args;
            ...
            pthread_create(&t1, NULL, prepare_sort, &left_args);
            prepare_sort(&right_args);
    ...
    {
        insertion_sort(prepared_args->block);
    }
}
