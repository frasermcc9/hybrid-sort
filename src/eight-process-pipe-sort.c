...
void prepare_sort(struct block *block, int depth)
{
    ...
    int pipe_size = fcntl(pipe_fds[1], F_SETPIPE_SZ, 1024 * 1024);
    if (pipe_size < 0)
    {
        perror("Pipe resize failure\n");
        exit(EXIT_FAILURE);
    }
    ...
    if (depth < 3)
    {
        ...
        if (pid != 0)
        {
            ...
            close(pipe_fds[1]);
            prepare_sort(&left_block, depth + 1);

            for (int i = 0; i < right_block.size; ++i)
            {
                if (read(pipe_fds[0], &right_block.data[i], sizeof(right_block.data[i])) == -1)
                {
        ...
        else
        {
            // in child
            close(pipe_fds[0]);
            prepare_sort(&right_block, depth + 1);

            for (int i = 0; i < right_block.size; ++i)
            {
                if (write(pipe_fds[1], &right_block.data[i], sizeof(right_block.data[i])) == -1)
                {
